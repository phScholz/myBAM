...
{
    if(ui->inputBox->toPlainText().toAscii()!= "" && ui->currentPath->text()!=""){
...
void bamWindow::readCountingInput(){
    qDebug() << "\nRead Input:";
    QString text=ui->inputBox->toPlainText().toAscii();
    QStringList lines;
...
void bamWindow::loadFile(){
    ...
    while(!in.atEnd()){
        ...
        line=in.readLine().toAscii().append("\n");
    ...
    text="";
    while(!in2.atEnd()){
        text+=in2.readLine().toAscii().append("\n");
    }
    qDebug() << text;
